...
const RenAIssanceComponent = () => {
  ...
  const handleSubmit = async () => {
    try {
      const res = await axios.post('http://localhost:3001/api/ask', { question }); // Cambia la URL según tu configuración de backend
  ...
  return (
    <div style={{ textAlign: 'center', marginTop: '50px', backgroundImage: `url(${require('./images/rnc.jpeg')})`, backgroundSize: 'cover', minHeight: '100vh', display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
      <div style={{ border: '3px solid #3f51b5', borderRadius: '4px', padding: '22px', maxWidth: '500px', margin: 'auto', backgroundColor: 'rgba(255, 255, 255, 0.8)' }}>
        ...
        <textarea
          ...
          placeholder=""
          style={{ width: '100%', minHeight: '100px', marginBottom: '10px', borderRadius: '5px', padding: '1px' }}
        />
        <br />
